...
function add_u(x) {
  ...
  if ((x.indexOf('https') >= 0) && (x.indexOf('.mp4') >= 0) || (x.indexOf('series') >= 0) || (x.indexOf('movies') >= 0)) {
    uls = x.replace('https', 'http');
    return uls;
  };
...
function youtube(x) {
  ...
  xhr.onload = function () {
    try {
      ...
      Y91(Load_timer);
      var player_config = xhr.response.match(/player.config\s*?=\s*?({.*?});/);
      jsUrl = 'https://www.youtube.com' + JSON.parse(player_config[1]).assets.js;
      var formatshls = JSON.parse(JSON.parse(player_config[1]).args.player_response).streamingData.hlsManifestUrl;
      alert("++formatshls++ " + formatshls);
      if (formatshls) {
        alert("++formatshls++ " + formatshls);
        R96("", "", formatshls);
      } else {
        var formatsArr = JSON.parse(JSON.parse(player_config[1]).args.player_response).streamingData.formats.reverse();
        formats = formatsArr[0];
        if (Boolean(formats.url)) {
          me = formats.url;
          R42(me)
        } else {
          if (Boolean(formats.cipher)) {
            formats.cipher = '&' + decodeURIComponent(formats.cipher);
            var url = formats.cipher.match(/url=(.*)/g).join('').replace(/url=/g, '').replace(/&sp=(.*)/g, '').replace(/&s=(.*)/g, '');
            var sig = formats.cipher.match(/&s=(.*)/g).join('').replace(/&s=/g, '').replace(/&sp=(.*)/g, '').replace(/&url=(.*)/g, '');
            youtubeGetAlgo(function (data) {
              sig = youtubeDecrypt(sig, data);
              me = url.concat('&sig=', sig);
              R42(me);
            })
          }
        };
...
function youtubeGetAlgo(algo) {
  var js = new XMLHttpRequest();
  js.open('GET', jsUrl, true);
  js.send();
  js.onload = function () {
    var jsObject = js.response.replace(/(\r\n|\r|\n)/g, '');
    var sFunAlgo = jsObject.match(/\b\w{2}=function\(a\)\{a=a\.split\(""\);(.*?);return/);
    var sFunChip = jsObject.match(new RegExp('var ' + sFunAlgo[1].match(/(\b\w{2})\./)[1] + '={(.*?})}'));
    var arr = [];
    sFunAlgo[1].split(';').forEach(function (elem) {
      var objAlgo = elem.replace(/(.*)\./, '').replace(/\((.*)/, '');
      var numAlgo = elem.replace(/(.*),/, '').replace(/(\))/, '');
      sFunChip[1].split('},').forEach(function (elem) {
        if (objAlgo == elem.replace(/:function(.*)/, '')) {
          if (elem.match('reverse')) {
            arr.push('r' + numAlgo)
          }
        };
        if (objAlgo == elem.replace(/:function(.*)/, '')) {
          if (elem.match('splice')) {
            arr.push('s' + numAlgo)
          }
        };
        if (objAlgo == elem.replace(/:function(.*)/, '')) {
...
function youtubeDecrypt(sig, algo) {
  algo.forEach(function (elem) {
    if (elem.length > 1) {
      var num = elem.slice(1)
    };
    if (num == undefined) {
      num == 0;
    };
...
function F51(ul, p, z) {
  alert('# F51 #' + ul);
  if (ul == 'md5hash' && Ach(12).indexOf('https://ok.ru/?id=getvideo') > 0) {
    ul = Ach(12).replace('?id=getvideo&v=', 'video/');
    return F51(ul)
  };
...
function R96(vid, j, ul) {
  if (vid) {
    ...
    if (!ul) {
      ul = "http://www.youtube.com/watch?v=" + tk[0] + "&hl=ru&persist_hl=1&has_verified=1";
      return youtube(ul);
    }
  };
  alert('# R96_ul #' + ul);
              ...
              if (ul["indexOf"]("vk.com/video_ext.php") > 0 || ul["indexOf"]("al_video.php") > 0) {
                ...
                if (ul["indexOf"]("vk.com/video") >= 0) {
                  ...
                  if (Tizen == true) {
                    if (vid) {
                      e = xhr["responseText"];
                      var test_post = R98(e, 'var ytplayer = ytplayer', '</div>');
                      t = R98(test_post, 'hlsManifestUrl', "index.m3u8");
                      if (t) {
                        t = t["replace"](/(\\|:|")/gm, "");
                        t = t["replace"]("https//", "https://");
                        return R96("", "", R95(t) + "index.m3u8")
                      };
                      if ((test_post.match(/sig=(.*)/) >= 0) || serC || serD) {
                        m = LOAD.postURL(_mx254[0xd], 'data=' + encodeURIComponent(test_post), '', '', '');
                        return youtube(ul)
                      };
                      t = R98(test_post, "url_encoded_fmt_stream_map", "</script>");
                      if (t["indexOf"]("itag") >= 0) {
                        if (tk[2] && tk[2] == "shark") {
      ...
      if (ne) {
        ...
        for (var k in arr) {
          if (arr[k]) {
            arr[k] = R95(arr[k]);
            alert("---arr[k]---" + arr[k]);
            if (arr[k]["indexOf"]("http") == 0) {
              r1 = arr[k];
              if (arr[k]["indexOf"](",quality") == 0) {
                s = R98(arr[k], 'http', ",quality");
                r1 = 'http' + s;
              }
            } else {
    ...
    if (xhr && xhr["readyState"] == 4) {
      ...
      if (xhr["status"] == 200) {
        ...
        if (t.indexOf('<div class="serial-seasons-box">') > 0 && ul.indexOf('pd_sign=') > 0 && ul.indexOf('/go/seria/') < 0 && ul.indexOf('season=') < 0) {
          ...
          F62(ul + '&season=' + ch, cls, "");
          return;
        } else if ((ul.indexOf('title') > 0 && ul.indexOf('kodikapi.com') >= 0) || ul.indexOf('kodikapi.com/list?token') > 0) {
          alert("==kodikapi==" + t);
          if (t.indexOf('link') < 0) {
            token = R98(ul, 'token=', '&');
            return xml_k('https://kodikapi.com/list?token=' + token)
          };
          token = R98(ul, 'token=', '&');
          df = t.split('type');
          next = R98(t, 'next_page":"', '"');
          prev = R98(t, 'prev_page":"', '"');
          cls["push"](["Что Будем Искать ?", "", "", "<div class='title' style='margin: 0 0 0 5px; position: relative; border: 0px solid #EBEBEB; color: #33CCFF; font-weight: bold; font-size: 25px;'><center>Поиск В Базе КОДИК</center></div>", "", 'https://kodikapi.com/list?token=' + token, "", "", "", "", "", "", "", "Введите искомое название"]);
          if (prev) {
            cls["push"](["Назад", prev, "", "Назад", "", "", "", "", "", "", "", "", ""])
          };
          if (next) {
            cls["push"](["Далее", next, "", "Далее", "", "", "", "", "", "", "", "", ""])
          };
          for (var i = 0; i < df["length"]; i++) {
            ...
            name = '',
            l = '';
            other_title = R98(res, 'other_title":"', '"');
            if (other_title) {
              name = other_title + '-'
            };
            title = R98(res, 'title":"', '"');
            ...
            year = R98(res, 'year":', ',');
            l = name;
            kinopoisk_id = R98(res, 'kinopoisk_id":"', '"');
            nt = R98(res, 'created_at":"', '"');
            p = "http://st.kp.yandex.net/images/film_big/" + kinopoisk_id + ".jpg";
            if (lin != "") {
              cls["push"]([lrdPr(title), lrdPr(ch), p, "<div class='film' style='float: left; margin: 0 0 0 5px; width: 100%; height: 380px;'><div class='title' style='margin: 0 0 0 5px; position: relative; border: 0px solid #EBEBEB; color: #33CCFF; font-weight: bold; font-size: 25px;'><center>" + l + "</center></div><div class='poster'><img style='float:left; margin: 9px 9px 0px 0px; width: 150px; height: 220px;border: 2px #33CCFF solid;' src='" + p + "'></div> <div class='about' style='font-size: 16px; padding: 0 15px 15px 0; position: relative;'><br><b style='color: #33CCFF; padding-right: 1px; font-weight: bold;'> </b><font color='BEBEBE'> </font><font color='33CCFF'> </font></div> <div class='genre' style='font-size: 16px;'> <b style='color: #33CCFF; font-weight: bold;'> Качество: </b><font color='BEBEBE'>" + quality + "</font></div>  <div class='genre' style='font-size: 16px;'> <b style='color: #33CCFF; font-weight: bold;'> ГОД: </b><font color='BEBEBE'>" + year + "</font></div> <div class='genre' style='font-size: 16px;'> <b style='color: #33CCFF; font-weight: bold;'> Добавлен: </b><font color='BEBEBE'>" + nt + "</font></div></div>", "", "", "", "", "", "", "", "", ""])
            }
          };
          F62(ch, cls, "");
          return;
...
LOAD.printPOST = function (x, set, token, ref, data) {
  ...
  if (ref.indexOf("filmix") >= 0) {
    ...
    if (ref.indexOf("videoframe.at") >= 0) {
      x.setRequestHeader("Host", j[2]),
      x.setRequestHeader("User-Agent", navigator.userAgent),
      x.setRequestHeader("Accept", "*/*"),
      x.setRequestHeader("Accept-Language", "ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3"),
      x.setRequestHeader("Accept-Encoding", "gzip, deflate, br"),
      x.setRequestHeader("Referer", 'https://' + j[2] + '/'),
      x.setRequestHeader("Content-Type", 'application/x-www-form-urlencoded; charset=UTF-8'),
      x.setRequestHeader("X-REF", j[2]),
      x.setRequestHeader("Origin", 'https://' + j[2] + '/'),
