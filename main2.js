  ...
  xhr["onreadystatechange"] = function () {
    if (xhr && xhr["readyState"] == 4) {
      ...
      if (xhr["status"] == 200) {
        ...
        data = JSON.parse(t);
        if (data.hlsUrl) {
          return R96("", "", data.hlsUrl);
        } else {
          data = data.formats.reverse();
          return R42(data[0].url);
        }
      } else {
...
function R96(vid, j, ul) {
  if (vid) {
    ...
    if (!ul) {
      if (typeof window.tizen !== "undefined" && typeof window.webapis !== "undefined") {
        return yout(_mx254[0x3] + _mx254[0x0] + '/youtube/parser.php?v=' + tk[0] + '&box_mac=' + P1);
      } else {
